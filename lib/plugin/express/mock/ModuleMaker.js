...
module.exports = class ModuleMaker extends EventEmitter {
  ...
  // 创建mock模块
  makeMockModule({ method, path: requestPath, data }) {
    try {
      ...
      if (rootDir) {
        // 处理restful动态参数
        const apiPath = helper.filterDynamicPathParams(requestPath)
        ...
        if (moduleDir) {
          ...
          // 创建模块文件
          this.makeModuleFile(`${dirName}/${lodash.lowerFirst(fileName)}`, {
            data,
            method,
            path: apiPath,
          })
