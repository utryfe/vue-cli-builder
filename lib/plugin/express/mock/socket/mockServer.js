...
class MockServer extends EventEmitter {
  ...
  createServer() {
    ...
    server.of('/', (socket) => {
      this.listen(socket)
      debug('listening root mock client.')
    })
    // sub
    server
      .of((name, query, next) => {
      ...
      .on('connect', (socket) => {
        this.listen(socket)
        debug('listening sub mock client.')
      })
    return server
  }
  ...
  send(message, connection) {
    debug(
      'sending message to mock client. [%s][%s]',
      message.type,
      message.path
    )
  ...
  reconnection(socket, connections) {
    for (const conn of connections) {
      const { token, url, channel, client } = conn
      this.send(
        {
          type: 'connection',
  ...
  incoming(connection) {
    const { token, url, channel, client } = connection
