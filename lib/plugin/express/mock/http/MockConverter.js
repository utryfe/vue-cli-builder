...
//
const URL_REG_EXP = /^\s*https?:\/\/\w(?:\.\w){1,3}/i
...
class MockConverter {
  ...
  //
  init() {
    let { path: mockPath, data, watch } = this.options
  ...
  watch(mockPath, absDataFile) {
    ...
    //
    commonUtil.watch(absDataFile, (event) => {
      if (event !== 'unlink') {
        MockConverter.writeMockTemplate(absDataFile, relDataFile, absTplFile, (err) => {
          if (!err) {
            logger.log(
...
// 转换JSON文件成Mock模板
MockConverter.writeMockTemplate = function(absDataFile, relDataFile, tplFile, callback) {
  let err = null
  let data = ''
  try {
    const dataString = fs
      .readFileSync(absDataFile)
      .toString('utf8')
      .trim()
    data = dataString ? JSON5.parse(dataString) : ''
  } catch (e) {
    err = e
    logger.error(`\n${e.message} [${relDataFile}]\n`)
  }
  if (!err) {
    try {
...
MockConverter.toArrayTemplate = function(name, value) {
  ...
  if (types.length === 1) {
    ...
    // 列表中为不同类型的元素
    // 进行单独转换
    return [name, value.map((item, index) => MockConverter.toTemplate(index, item)[1])]
  }
}
