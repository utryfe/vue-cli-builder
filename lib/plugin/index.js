...
class Plugin {
  ...
  ensureProxyContext(context) {
    if (
      ...
      context = '^/(?!sockjs-node/).+'
    } else if ('/sockjs-node/'.match(context)) {
      // 排除对开发服务器socket的代理转发
      context = `(?:^(?!/sockjs-node/))${context}`
    }
    return context
  }
}
...
Plugin.registerPlugin = function(name, plugin) {
  if (typeof name !== 'string') {
    logger.error(`[registerPlugin] The type of name must be a string. (${name}`)
    process.exit(1)
  }
  const plugins = Plugin.plugins
  const lowerName = name.toLowerCase()
  if (Object.prototype.hasOwnProperty.call(plugins, lowerName)) {
    logger.error(`[registerPlugin] The plugin name of '${name}' already exists.`)
    process.exit(1)
  }
  ...
  if (typeof plugin !== 'function') {
    logger.error(`[registerPlugin] Plugin must be a function. (${name})`)
    process.exit(1)
  }
...
Plugin.getPlugin = function(name) {
  ...
  logger.error(`[getPlugin] Plugin load error. (${name})`)
  process.exit(1)
}
