...
module.exports = exports = {
  ...
  // 遍历树（深度优先）
  transverseTree(nodes, childrenProp, callback, parent, root) {
    ...
    for (const node of nodes) {
      ...
      if (
        res === false ||
        exports.transverseTree(children, childrenProp, callback, node, root) === false
      ) {
        return false
      }
      if (res === 'flat') {
  ...
  getIdentifierMaker(type = 'id', init) {
    ...
    return (namespace = 'space') => {
      const identifier = `${type}${lodash.upperFirst(
        namespace !== '/'
  ...
  watch(pattern, callback, options) {
    ...
    const method = ['debounce', 'throttle'].includes(setup.method)
      ? setup.method
      : 'debounce'
    delete setup.delay
    delete setup.method
    ...
    if (callback && typeof callback === 'object') {
      Object.keys(callback).forEach((event) => {
        ...
        if (typeof handler === 'function') {
          watcher.on(event, delay ? lodash[method](handler, delay, delaySetup) : handler)
        }
      })
    } else if (typeof callback === 'function') {
      watcher.on('all', delay ? lodash[method](callback, delay, delaySetup) : callback)
    }
    //
