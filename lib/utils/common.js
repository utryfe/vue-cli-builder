...
const utils = {
  ...
  async printListeningAddress(server, extra, boxen) {
    ...
    if (typeof details === 'string') {
      ...
      localAddress = `http://${address === '0.0.0.0' ? 'localhost' : address}:${port}`
  ...
  async createLocalHttpServer(port, handler, closed) {
    const actualPort = await utils.getNetworkPort(+port || 5000)
    const httpServer = require('http').createServer(handler)
    require('killable')(httpServer)
    //
    return new Promise((resolve, reject) => {
      httpServer.listen(actualPort, '0.0.0.0', async (err) => {
        if (!err) {
          utils.registerShutdown(() => {
            httpServer.kill(closed)
          })
          const ip = await utils.getNetworkHostIP()
          resolve({ server: httpServer, port: actualPort, ip })
        } else {
          reject(err)
        }
      })
