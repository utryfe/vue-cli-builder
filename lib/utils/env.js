...
process.env.UT_BUILD_COMMAND_TIMESTAMP =
  process.env.UT_BUILD_COMMAND_TIMESTAMP || Date.now()
...
process.env.UT_BUILD_COMMAND_LIST_STRING = `${(
  process.env.UT_BUILD_COMMAND_LIST || ''
).trim()}_${command}`

process.UT_BUILD_COMMAND_LIST = process.env.UT_BUILD_COMMAND_LIST_STRING.split('_')
  .map((cmd) => cmd.trim())
  .filter((cmd) => !!cmd)
...
exports.commandList = [...process['UT_BUILD_COMMAND_LIST']]
// 内部环境变量
exports.ENV = ENV
// 是否是帮助命令模式
