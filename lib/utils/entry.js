...
function getMultiPages() {
  const pages = {}
  const { BUILD_MPA_ENTRY, BUILD_MPA, BUILD_HTML_TEMPLATE } = env()
  const entry =
    typeof BUILD_MPA_ENTRY === 'string' ? BUILD_MPA_ENTRY.split(',') : []
  const targets =
...
module.exports = {
  ...
  toPages(options) {
    ...
    entryPages = {}
    const { BUILD_MPA, BUILD_SPA, BUILD_HTML_TEMPLATE, BUILD_SPA_ENTRY } = env()
