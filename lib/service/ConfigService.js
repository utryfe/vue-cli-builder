...
class ConfigService {
  ...
  chainWebpack() {
    ...
    this.plugin.chainWebpack((chainConfig) => {
      ...
      // 应用于拦截的默认链式配置
      ConfigService.chainDefaultWebpack(chainConfig, pluginSetup)
    })
  }
...
// 预处理设置相关
function usePreprocessPlugin(chainConfig, pluginOptions) {
  const { preprocess } = pluginOptions
  if (preprocess) {
    const jsRule = chainConfig.module.rule('js')
    const includeHandler = (filePath) => {
      return /[/\\]node_modules[/\\]\.code[/\\].+?\.js$/.test(filePath)
    }
...
// 配置默认的chain操作
ConfigService.chainDefaultWebpack = function(chainConfig, pluginOptions) {
  usePreprocessPlugin(chainConfig, pluginOptions)
