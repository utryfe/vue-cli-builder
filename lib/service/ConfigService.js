...
class ConfigService {
  ...
  constructor(setup) {
    ...
    try {
      // 安装了icefox包，则添加转译支持
      const icefox = require.resolve('icefox').replace(/\\/g, '/')
      const iceLib = fileUtil
        .joinPath(icefox.replace(/index\.js$/, ''), 'lib')
        .replace(/\\/g, '/')

      ConfigService.addTranspileDependency((path) => {
        const normalPath = path.replace(/\\/g, '/')
        return normalPath === icefox || normalPath.startsWith(iceLib)
      })
    } catch (e) {
      debug('icefox not installed')
    }
  }
