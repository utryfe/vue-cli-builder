...
// web应用服务器
function serveApplication(app, publicPath, servePath, devServer) {
  // html5 历史记录路由 404 服务
  let { historyApiFallback } = Object.assign({}, devServer)
  if (!historyApiFallback) {
    historyApiFallback = getEnv.args['history-api-fallback']
  }
...
exports = module.exports = (api, projectOptions) => (args = {}, rawArgv = []) => {
  ...
  return runTask.then(async () => {
    ...
    // web应用
    serveApplication(app, publicPath, servePath, projectOptions.devServer)
