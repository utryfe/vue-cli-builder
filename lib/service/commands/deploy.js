...
function printDetails(details, spinner) {
  ...
  const message = commonUtil.prettyPrintPaths(
    [
      ...
      {
        type: 'Backup Path',
        path: backup || chalk['bgRed']['white']('no need to backup (--no-backup)'),
      },
      {
        type: 'Write Mode',
        path: overwrite
          ? 'overwrite only'
          : chalk['bgRed']['white']('delete before write (--no-overwrite)'),
      },
    ],
    0,
    '- '
    ...
    if (targetPath) {
      ...
      if (!path) {
        ...
        if (interactive) {
          ...
          spinner.info(
            'Automatically create non-existent paths due to interactive enabled.\n'
          )
          mkdir = true
        }
...
async function execBackup(setup) {
  ...
  spinner.succeed(`Backup upon server successfully completed: ${chalk['cyan'](zip)}\n`)
...
async function decompress(co, src, spinner) {
  ...
  spinner.succeed('Decompressed successfully.\n')
...
async function execTransferFiles(setup) {
  ...
  await co
    .upload(src, tmpTarget, (progress) => {
      spinner.text = `Transferring... (${progress}%)`
...
async function resolveConfig(spinner) {
  ...
  try {
    if (fileUtil.existsSync(absConfigFile) && !fileUtil.isDirectory(absConfigFile)) {
      spinner.info(
        `Read the configuration from: ${color(
          `***/${fileUtil.getFileBaseName(absConfigFile)}`
        )}\n`
      )

      const file = await promisify(fs.readFile)(absConfigFile, { encoding: 'utf8' })
      // 可使用base64、hex简单屏蔽明文配置
      let str
      try {
        if (Buffer.isEncoding('base64')) {
          str = new Buffer(file, 'base64').toString()
        } else if (Buffer.isEncoding('hex')) {
          str = new Buffer(file, 'hex').toString()
        } else {
          str = file
        }
      } catch (e) {
        debug('decode configuration file error: %s', e.message)
      }

      configArgs = str ? require('json5').parse(str) : null
      const { username, password, privateKey, deployPath } = Object.assign({}, configArgs)
      configArgs = Object.assign(
        {
          // 使用配置文件时，默认禁用交互模式
          suspend: false,
          interactive: false,
    ...
    spinner.log(`${chalk['red'](e.message)}\n`)
    spinner.fail(
      `${chalk['red'](
...
exports = module.exports = (api, projectOptions) => {
  ...
  return async () => {
    ...
    if (success) {
      ...
      //
      spinner.succeed(
        `${chalk['cyan']('The deployment operation has been successfully completed.')}\n`
      )
    } else {
