...
function printDetails(details, spinner) {
  ...
  const message = commonUtil.prettyPrintPaths(
    [
      ...
      {
        type: 'Backup Path',
        path: backup || chalk['bgRed']['black']('no need to backup (--no-backup)'),
      },
      {
        type: 'Write Mode',
        path: overwrite
          ? 'overwrite only'
          : chalk['bgRed']['black']('delete before write (--no-overwrite)'),
      },
    ],
    0,
    '- '
...
async function execTransferFiles(setup) {
  ...
  try {
    ...
    await co
      .execWithPipe(true, `rm -vrf ${escapedTarget}`)
