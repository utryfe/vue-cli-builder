...
function setModuleNormalProps(module, nestedType, context) {
  ...
  if (!isRoot && children && nestedType !== 'none') {
    const nestedReg = /^~?\[.*?]$/
    const rootNestedReg = /^~.+/
    const baseName = getFileBaseName(pathname)
    module.autoNested = nestedType === 'auto'
    module.manualNested = nestedReg.test(baseName)
    module.declareNested = module.autoNested || module.manualNested
