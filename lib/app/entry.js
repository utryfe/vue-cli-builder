...
class EntryManager {
  constructor(projectOptions) {
    ...
    this.env.registerVariables('UT_BUILD_ENTRY_SETUP', entrySetup)
    // 路由模式
    if (preprocess) {
      const {
      ...
      if (useRouter && routerMode === 'history') {
        ConfigService.addChainWebpack((config) => {
          ...
          if (!devServer.get('historyApiFallback')) {
            ...
            getEnv.ENV['history-api-fallback'] = true
          }
        })
      }
    }
  ...
  // 获取构建模块
  getBuildModules() {
    const { BUILD_MODULE_ENTRY, BUILD_MODULE_FILTER, BUILD_ROOT_APP_PATH } = this.config
  ...
  buildLegacyEntry(modules, entryPages) {
    ...
    modules.forEach((module) => {
      ...
      //
      const entry = fileUtil.isAbsolute(module) ? module : fileUtil.resolvePath(module)
      //
  ...
  buildSPAEntry(modules, entryPages) {
    const { config, options, publicPath } = this
    const { BUILD_HTML_TEMPLATE } = config
    const { indexPath } = options
    //
    const entryPoints = generate({
