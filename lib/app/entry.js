...
class EntryManager {
  constructor(options) {
    ...
    const { preprocess } = Object.assign({}, options.pluginOptions)
    this.env = getEnv()
  ...
  formatEntries(entries) {
    ...
    if (!entryPoints.length) {
      // 没有入口文件，则退出
      logger.error('Must include at least one entry.')
      process.exit(1)
    }
    // 修改名称
  ...
  buildLegacyEntry(modules, entryPages) {
    ...
    modules.forEach((module) => {
      //
      const moduleName =
        length === 1 ? 'index' : fileUtil.getShortDirName(module).toLowerCase()
      const fileName =
  ...
  modifyEntryName(pages, points) {
    const { outputDir, indexPath, pluginOptions } = this.options
    ...
    if (typeof indexPath === 'string') {
      const dirName = fileUtil.getDirName(indexPath, outputDir)
      if (dirName) {
        Object.keys(pages).forEach((name) => {
          const page = pages[name]
          page.filename = fileUtil.joinPath(dirName, page.filename)
        })
      }
    }
  }
